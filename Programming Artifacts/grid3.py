    ...
    def updateTable(self, table, index):
        if self.edit_test_dialog.exec_():
            if table is self.tests_table:
                ...
                print("Age", self.Age_cb.isChecked())
                print("Education_Num", self.Education_Num_cb.isChecked())
                print("Hours_per_week", self.Hours_per_week_cb.isChecked())

                if self.edit_test_dialog.Sex_cb.isChecked() and self.edit_test_dialog.Race_cb.isChecked() and self.edit_test_dialog.Age_cb.isChecked() and self.edit_test_dialog.Education_Num_cb.isChecked() and self.edit_test_dialog.Hours_per_week_cb.isChecked():
...
class EditTestWindow(QDialog):
    ...
    def createGrid(self, name=None, conf=None, margin=None, test=None):
        ...
        self.Age_cb = QCheckBox("Age")
        self.Education_Num_cb = QCheckBox("Education_Num")
        self.Hours_per_week_cb = QCheckBox("Hours_per_week")

        self.Sex_cb.stateChanged.connect(self.selectionChange)
        self.Race_cb.stateChanged.connect(self.selectionChange)
        self.Age_cb.stateChanged.connect(self.selectionChange)
        self.Education_Num_cb.stateChanged.connect(self.selectionChange)
        self.Hours_per_week_cb.stateChanged.connect(self.selectionChange)

        self.layout.addWidget(self.inputs_label, 5, 1)
        self.layout.addWidget(self.Sex_cb, 5, 2)
        self.layout.addWidget(self.Race_cb, 6, 2)
        self.layout.addWidget(self.Age_cb, 7, 2)
        self.layout.addWidget(self.Education_Num_cb, 8, 2)
        self.layout.addWidget(self.Hours_per_week_cb, 9, 2)
    ...
    def selectionChange(self):
        if self.group_cb.isChecked or self.causal_cb.isChecked():
            if self.Sex_cb.isChecked() and self.Race_cb.isChecked() and self.Age_cb.isChecked() and self.Education_Num_cb.isChecked() and self.Hours_per_week_cb.isChecked():
                self.enterThreshold()
            else:
                try:
    ...
    def handleAddButton(self):
        ...
        if self.Race_cb.isChecked():
            inputs.append("Race")
        if self.Age_cb.isChecked():
            inputs.append("Age")
        if self.Education_Num_cb.isChecked():
            inputs.append("Education_Num")
        if self.Hours_per_week_cb.isChecked():
            inputs.append("Hours_per_week")
