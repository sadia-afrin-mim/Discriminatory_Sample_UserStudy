...
from detailed_output import print_detailed_discrimination
from causal_logger import write_causal_pairs
...
##        try:
        ...
        for fixed_sub_assign in self._gen_all_sub_inputs(args=i_fields):
            ...
            #print(len(i_fields))
            if(len(i_fields)==2):
             #print(fixed_sub_assign[i_fields[0]] + "," +fixed_sub_assign[i_fields[1]]+"--> " + str(p))
...
##            print("Issue in group_discrimination")
    ...
    def causal_discrimination(self, i_fields=None, conf=0.999, margin=0.0001):
        print("[DEBUG] Entered causal_discrimination")  # ✅ ADD HERE
...
##        try:
        ...
        causal_pairs = []
        print("Assigning values to see the causal effect")
        for num_sampled in range(1, self.max_samples):
    ...
    # ---- Call it after computing attribute_effects ----
        ...
        for sub in self._all_relevant_subs(self.input_order):
            ...
            if causal:
                suite, p, cp = self.causal_discrimination(i_fields=sub, conf=conf,
                                                   margin=margin)
                print("**** Results for Causal Test****")
                print (sub)
                print(conf)
                print(margin)
                print (p)
                print("Example causal tests that changes the result")
                print (cp)
                # writing causal unit test cases:
                if cp:
                    write_causal_pairs(cp, output_file="causal_pairs_tests.py", system_name="loan.py")
...
##            print("Issue in getting the results of the tests")
    ...
    def _merge_assignments(self, assign1, assign2):
        try:
            ...
            print("Issue in merging assignments")
...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument("XML_FILE", type=str, nargs=1, help="XML configuration file")
        #parser.add_argument("--detailed", action="store_true", help="Print detailed discrimination output")
       ...
       # if args.detailed:
            #print_detailed_discrimination(t)  # ✅ Call the CLI output
