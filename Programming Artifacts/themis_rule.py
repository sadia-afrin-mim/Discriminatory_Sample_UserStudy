...
import importlib
import themis2_timing_updated
importlib.reload(themis2_timing_updated)

themis2 = themis2_timing_updated

#import themis2_timing_updated as themis2#$import os
...
class App(QDialog):
    ...
    def handleRunButton(self):
        ...
        if hasattr(self.tester, "first_disc_time") and self.tester.first_disc_time is not None:
            self.results_box.append(
                f"<h3>⏱ Time to first discriminatory sample: {self.tester.first_disc_time:.4f} seconds</h3>"
            )
        else:
            self.results_box.append(
    ...
    # MOVED out of handleRunButton: LLM helper kept intact
    def _llm_explain(self, results_text: str) -> str:
        """
        Returns a short explanation using an LLM. Uses OPENAI_API_KEY from env.
        Works with both new (OpenAI) and legacy (openai) SDKs.
        """
        api_key = os.getenv("OPENAI_API_KEY")
        if not api_key:
            return "No OPENAI_API_KEY found. Please set it in your environment."

        prompt = (
            "You are a fairness auditor. Explain these Themis discrimination results "
            "for a non-expert. Cover:\n"
            "• What group vs causal discrimination means\n"
            "• Which attributes triggered flags and why\n"
            "• What min/max scores imply\n"
            "• 1–2 concrete mitigation ideas (thresholds, prompt changes, post-processing)\n"
            "Keep it under 180 words. Be clear and neutral.\n\n"
            f"RESULTS:\n{results_text}\n"
        )
    ...
    def handleDetailedButton(self):
        ...
        # extra box to hold LLM explanation (keeps original results intact)
        explain_box = QTextEdit()
        explain_box.setReadOnly(True)
        explain_box.setPlaceholderText(
            "Click 'Explain with LLM' to generate a short explanation.\n"
            "Your detailed results above will remain unchanged."
        )

        # --- populate detailed_output_box ---
        for test in self.tester.tests:
            if test.group == True or test.causal == True:
                        ...
                        for result in self.tester.group_measurement_results:
                            if (key == tuple(result.i_fields)):
                                ...
                                detailed_output_box.append(
                                    "&nbsp; &nbsp; &nbsp; <b>Min: </b>" + min_assign + " &#8594; " + "{:.1%}".format(min_score))
                                detailed_output_box.append(
                                    "&nbsp; &nbsp; &nbsp; <b>Max: </b>" + max_assign + " &#8594; " + "{:.1%}".format(max_score))
        ...
        # buttons row
        btn_explain = QPushButton("Explain with LLM")
        btn_close = QPushButton("Close")

        def _do_explain():
            # keep existing details; read their plain text
            results_text = detailed_output_box.toPlainText()

            # keep the FIRST half
            half_text = results_text[:len(results_text) // 8]
            explain_box.setPlainText("Generating explanation...")
            text = self._llm_explain(half_text)
            explain_box.setPlainText(text)

        btn_explain.clicked.connect(_do_explain)
        btn_close.clicked.connect(dialog.close)

        # layout
        layout.addWidget(detailed_output_box, 0, 0, 1, 2)
        layout.addWidget(btn_explain, 1, 0, 1, 1)
        layout.addWidget(btn_close, 1, 1, 1, 1)
        layout.addWidget(explain_box, 2, 0, 1, 2)
        ...
        # --- ORIGINAL EXTRA LINES (kept but disabled to avoid duplicate layouts) ---
    ...
    def updateTable(self, table, index):
        if self.edit_test_dialog.exec_():
            if table is self.tests_table:
                ...
                # NOTE: the following self.sex_cb/self.race_cb/self.income_cb
                # belong to EditTestWindow, not App. Leaving as-is (original logic),
                # but this may raise AttributeError. Consider wiring values from the dialog.
