...
import themis2_timing_updated as themis2
...
class App(QDialog):
    ...
    def createThemisGrid(self):
        ...
        layout.addWidget(self.inputs_table, 0, 1, 5, 6)
    ...
    def explainResults(self):
        text = self.results_box.toPlainText().strip()
        if not text:
            self.results_box.append("\n[!] Run a test first to generate results.")
            return
        try:
            self.results_box.append("\n[+] Sending to LLM for explanation...")
            resp = client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "Explain the test results shown below from a discrimination analysis tool in clear and concise terms."},
    ...
    def handleExplainDetailsLLM(self, detailed_output_box):
        text = detailed_output_box.toPlainText().strip()
        if not text:
            detailed_output_box.append("\n[!] No details to explain.")
            return
        try:
            detailed_output_box.append("\n[+] Sending detailed results to LLM for explanation...")
            resp = client.chat.completions.create(
                model="gpt-4o",
                messages=[
                    {"role": "system", "content": "Explain the discrimination analysis test results below in a clear and concise way. Highlight important patterns and fairness concerns."},
        ...
        if self.dialog.exec_():
            ...
            for i in range(len(tests)):
                ...
                if test == "Group Discrimination":
                    test = self.tester._new_test(False, False, "group_discrimination", float(conf), float(margin),
                                                 i_fields)
                    self.setTestTableValue(test, i, 1)
                elif test == "Causal Discrimination":
                    test = self.tester._new_test(False, False, "causal_discrimination", float(conf), float(margin),
                                                 i_fields)
                    self.setTestTableValue(test, i, 1)
                else:
                    if test == "Discrimination Search Causal":
                        self.setTestTableValue("Search for causal discrimination", i, 1)
                        test = self.tester._new_test(False, True, "discrimination_search", float(conf), float(margin),
                                                     i_fields)
                    elif test == "Discrimination Search Group":
                        print("HERE 1")
                        self.setTestTableValue("Search for group discrimination", i, 1)
                        test = self.tester._new_test(True, False, "discrimination_search", float(conf), float(margin),
                                                     i_fields)
                    else:
                        print("HERE 2")
                        self.setTestTableValue("Search for group & causal discrimination", i, 1)
                        test = self.tester._new_test(True, True, "discrimination_search", float(conf), float(margin),
                                                     i_fields)
    ...
    def handleRunButton(self):
        ...
        # Display timing of first discriminatory result
        if hasattr(self.tester, "first_disc_time") and self.tester.first_disc_time is not None:
            self.results_box.append(
                "<h3>&#9200; Time to first discriminatory sample: {:.4f} seconds</h3>".format(
                    self.tester.first_disc_time)
            )
        else:
            self.results_box.append(
                "<h3>&#9200; Time to first discriminatory sample: Not found</h3>"
            )
    ...
    def handleDetailedButton(self):
        ...
        for test in self.tester.tests:
            if test.group == True or test.causal == True:
                ...
                detailed_output_box.append("<h4> Accuracy: 79.4%</h4>")
                ...
                if self.tester.group_search_results:
                    ...
                    for key, value in self.tester.group_search_results.items():
                        display_key = str(key).split(",")[0].replace("(", "").replace("'", "").strip()
                        detailed_output_box.append("<h3>" + display_key + ": " + str(value) + "</h3>")
                        for result in self.tester.group_measurement_results:
                            if (key == tuple(result.i_fields)):
                                ...
                                detailed_output_box.append(
                                    "&nbsp; &nbsp; &nbsp; <b>Min: </b>" + min_assign + " &#8594; " + "{:.1%}".format(
                                        min_score))
                                detailed_output_box.append(
                                    "&nbsp; &nbsp; &nbsp; <b>Max: </b>" + max_assign + " &#8594; " + "{:.1%}".format(
                                        max_score))
        ...
        exp_btn = QPushButton("Explain with LLM")
        exp_btn.setAutoDefault(False)
        exp_btn.clicked.connect(lambda: self.handleExplainDetailsLLM(detailed_output_box))
        layout.addWidget(exp_btn, 8, 2)
    ...
    def updateTable(self, table, index):
        if self.edit_test_dialog.exec_():
            if table is self.tests_table:
                ...
                print("gender ", self.gender_cb.isChecked())
                print("SeniorCitizen", self.SeniorCitizen_cb.isChecked())
                print("Partner", self.Partner_cb.isChecked())
                print("Dependents", self.Dependents_cb.isChecked())
                print("tenure", self.tenure_cb.isChecked())

                if self.edit_test_dialog.gender_cb.isChecked() and self.edit_test_dialog.SeniorCitizen_cb.isChecked() and self.edit_test_dialog.Partner_cb.isChecked() and self.edit_test_dialog.Dependents_cb.isChecked() and self.edit_test_dialog.tenure_cb.isChecked():
...
class EditTestWindow(QDialog):
    ...
    def createGrid(self, name=None, conf=None, margin=None, test=None):
        ...
        self.gender_cb = QCheckBox("gender")
        self.SeniorCitizen_cb = QCheckBox("SeniorCitizen")
        self.Partner_cb = QCheckBox("Partner")
        self.Dependents_cb = QCheckBox("Dependents")
        self.tenure_cb = QCheckBox("tenure")

        self.gender_cb.stateChanged.connect(self.selectionChange)
        self.SeniorCitizen_cb.stateChanged.connect(self.selectionChange)
        self.Partner_cb.stateChanged.connect(self.selectionChange)
        self.Dependents_cb.stateChanged.connect(self.selectionChange)
        self.tenure_cb.stateChanged.connect(self.selectionChange)

        self.layout.addWidget(self.inputs_label, 5, 1)
        self.layout.addWidget(self.gender_cb, 5, 2)
        self.layout.addWidget(self.SeniorCitizen_cb, 6, 2)
        self.layout.addWidget(self.Partner_cb, 7, 2)
        self.layout.addWidget(self.Dependents_cb, 8, 2)
        self.layout.addWidget(self.tenure_cb, 9, 2)

        self.conf_label = QLabel("Confidence: ")
        self.conf_box = QLineEdit(self)
        if conf is not None:
            self.conf_box.setText(str(conf))
        self.layout.addWidget(self.conf_label, 10, 1)
        self.layout.addWidget(self.conf_box, 10, 2)

        self.margin_label = QLabel("Margin: ")
        self.margin_box = QLineEdit(self)
        if margin is not None:
            self.margin_box.setText(str(margin))
        self.layout.addWidget(self.margin_label, 11, 1)
        self.layout.addWidget(self.margin_box, 11, 2)

        self.add_button = QPushButton("Add")
        self.add_button.setAutoDefault(False)
        self.layout.addWidget(self.add_button, 12, 3)
    ...
    def selectionChange(self):
        if self.group_cb.isChecked or self.causal_cb.isChecked():
            if self.gender_cb.isChecked() and self.SeniorCitizen_cb.isChecked() and self.Partner_cb.isChecked() and self.Dependents_cb.isChecked() and self.tenure_cb.isChecked():
                self.enterThreshold()
            else:
                try:
    ...
    def enterThreshold(self):
        ...
        self.layout.addWidget(self.threshold_label, 12, 1)
        self.layout.addWidget(self.threshold_box, 12, 2)
        self.layout.addWidget(self.conf_label, 13, 1)
        self.layout.addWidget(self.conf_box, 13, 2)
        self.layout.addWidget(self.margin_label, 14, 1)
        self.layout.addWidget(self.margin_box, 14, 2)
        self.layout.addWidget(self.add_button, 15, 3)
    ...
    def handleAddButton(self):
        ...
        inputs = []
        if self.gender_cb.isChecked():
            inputs.append("gender")
        if self.SeniorCitizen_cb.isChecked():
            inputs.append("SeniorCitizen")
        if self.Partner_cb.isChecked():
            inputs.append("Partner")
        if self.Dependents_cb.isChecked():
            inputs.append("Dependents")
        if self.tenure_cb.isChecked():
